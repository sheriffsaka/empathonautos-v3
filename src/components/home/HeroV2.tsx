...
export const HeroV2 = () => {
  ...
  return (
    <div className="relative h-[90vh] min-h-[600px] w-full overflow-hidden bg-[#030d22]">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#030d22] via-[#030d22]/40 to-transparent" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#030d22] via-transparent to-transparent opacity-60" />
        </motion.div>
      </AnimatePresence>
      ...
      <div className="relative z-10 h-full max-w-7xl mx-auto px-6 flex flex-col justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="max-w-2xl"
          >
            <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#80C5FC] mb-6 italic underline underline-offset-8 decoration-[#80C5FC]/30">
            ...
            <p className="text-white/70 text-lg md:text-xl max-w-md font-medium leading-relaxed mb-12">
              {slides[current].description}
            </p>
            <div className="flex flex-wrap gap-4">
              <button className="bg-[#80C5FC] text-[#030d22] px-10 py-5 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all flex items-center gap-3 shadow-[0_0_15px_rgba(128,197,252,0.3)]">
                Explore Fleet <ArrowRight className="w-4 h-4" />
              </button>
              <button className="bg-white/5 backdrop-blur-sm border border-[#80C5FC]/30 text-white px-10 py-5 text-[10px] font-black uppercase tracking-widest hover:bg-[#80C5FC] hover:text-[#030d22] transition-all">
                Pre-Order Now
              </button>
            </div>
          </motion.div>
      ...
      <div className="absolute bottom-12 right-6 md:right-12 z-20 flex flex-col md:flex-row items-center gap-8">
          ...
          {slides.map((_, i) => (
            <button
              ...
              className={cn(
                ...
                current === i ? "w-12 bg-[#80C5FC]" : "w-6 bg-white/20"
              )}
            />
          ))}
        </div>
        <div className="flex gap-1">
          <button 
            onClick={prev}
            className="p-4 bg-white/5 hover:bg-[#80C5FC] text-white hover:text-[#030d22] border border-[#80C5FC]/20 transition-all"
          >
